...
# 3x6 shape templates for the "42" glyph, read only at import time to
# decide WHICH cells belong to it: "#" = a glyph cell, "." = not.
# These characters are internal only -- they never appear in the grid,
# the output file, or any display. A glyph cell simply becomes an
...
_GLYPH_GAP = 1                            # empty columns between digits
_GLYPH_W = _DIGIT_W + _GLYPH_GAP + _DIGIT_W   # total glyph width  (7)
_GLYPH_H = _DIGIT_H                            # total glyph height (6)
...
class MazeGenerator:
    ...
    def generate(self) -> None:
        ...
        Stamps the "42" glyph (when it fits) and carves a spanning tree
    ...
    def _is_free(self, x: int, y: int) -> bool:
        ...
        return self._in_bounds(x, y) and (x, y) not in self._glyph
    ...
    def _place_glyph(self) -> None:
        ...
        The glyph needs a one-cell margin on every side. It is skipped
        ...
        the subject allows omitting the glyph when it cannot be placed.
        """
        self._glyph = set()
        if self.width < _GLYPH_W + 2 or self.height < _GLYPH_H + 2:
            print("maze too small for the '42' glyph; skipping it")
            return
        ox = (self.width - _GLYPH_W) // 2
        oy = (self.height - _GLYPH_H) // 2
        cells = self._digit_cells(_DIGIT_4, ox, oy)
        cells |= self._digit_cells(
            _DIGIT_2, ox + _DIGIT_W + _GLYPH_GAP, oy
        )
        if self._entry in cells or self._exit in cells:
            print("'42' glyph would cover entry/exit; skipping it")
            return
        if not self._region_connected_without(cells):
            print("'42' glyph would split the maze; skipping it")
            return
        self._glyph = cells
        full = N | E | S | W
        for x, y in cells:
    ...
    def _region_connected_without(self, blocked: set[Coord]) -> bool:
        ...
        cell around the glyph from any side.
        """
        free: set[Coord] = {
            (x, y)
            for y in range(self.height)
